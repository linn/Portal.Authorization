...
if [ "${TRAVIS_BRANCH}" = "main" ]; then
  if [ "${TRAVIS_PULL_REQUEST}" = "false" ]; then
    ...
    aws s3 cp s3://$S3_BUCKET_NAME/portalAuthorization/production.env ./secrets.env

    STACK_NAME=portalAuthorization
    ...
    aws s3 cp s3://$S3_BUCKET_NAME/portalAuthorization/sys.env ./secrets.env

    STACK_NAME=portalAuthorization-sys
